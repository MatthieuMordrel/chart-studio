...
describe('layout-options', () => {
  it('adds edge-aware layered defaults for cleaner graph layouts', () => {
    ...
    expect(options['elk.spacing.edgeNode']).toBe('34')
    expect(options['elk.layered.spacing.edgeNodeBetweenLayers']).toBe('32')
    expect(options['elk.layered.spacing.edgeEdgeBetweenLayers']).toBe('23')
  })
})
