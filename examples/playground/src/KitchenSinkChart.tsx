...
const DEFAULT_VISIBILITY: Record<ControlId, ControlVisibility> = {
  source: 'hidden', xAxis: 'hidden', chartType: 'pinned', groupBy: 'pinned',
  timeBucket: 'pinned', metric: 'overflow', filters: 'overflow', dateRange: 'overflow',
}
...
const PRESETS: readonly Preset[] = [
  ...
  },
  {
    ...
    label: 'All overflow',
    visibility: {
      source: 'overflow', xAxis: 'overflow', chartType: 'overflow', groupBy: 'overflow',
      timeBucket: 'overflow', metric: 'overflow', filters: 'overflow', dateRange: 'overflow',
    },
  },
  {
    id: 'all-pinned',
    label: 'All pinned',
    visibility: {
      source: 'pinned', xAxis: 'pinned', chartType: 'pinned', groupBy: 'pinned',
      timeBucket: 'pinned', metric: 'pinned', filters: 'pinned', dateRange: 'pinned',
    },
  },
]
...
export function KitchenSinkChart() {
  ...
  const pinned = useMemo(
    () => CONTROLS.filter(c => visibility[c.id] === 'pinned').map(c => c.id),
    [visibility],
  )
  const hidden = useMemo(
    () => CONTROLS.filter(c => visibility[c.id] === 'hidden').map(c => c.id),
    [visibility],
  )
  ...
  const setControl = (id: ControlId, state: ControlVisibility) =>
    setVisibility(prev => ({...prev, [id]: state}))
          ...
          <div className="overflow-hidden rounded-2xl border border-border bg-background shadow-sm">
            ...
            <div className="border-t border-border bg-card/30 px-4 py-3">
              <p className="text-[11px] leading-relaxed text-muted-foreground">
                In your app, pass <code className="font-semibold">pinned</code> and{' '}
                <code className="font-semibold">hidden</code> to{' '}
                <code className="font-semibold">{'<ChartToolbar />'}</code>. Unlisted controls
                default to the ellipsis menu.
              </p>
            </div>
          </div>
        </div>
