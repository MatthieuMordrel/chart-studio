      ...
      <div className="flex flex-wrap gap-4 rounded-2xl border border-border bg-background p-4">
        ...
        <label className="flex flex-col gap-1">
          ...
          <select
            ...
            onChange={e =>
              chart.setGroupBy(
                (e.target.value || null) as Parameters<typeof chart.setGroupBy>[0],
              )
            }
            className="rounded border border-border bg-card px-2 py-1.5 text-sm text-foreground"
          >
        ...
        <label className="flex flex-col gap-1">
          ...
          <select
            ...
            onChange={e =>
              chart.setTimeBucket(e.target.value as Parameters<typeof chart.setTimeBucket>[0])
            }
            className="rounded border border-border bg-card px-2 py-1.5 text-sm text-foreground"
              ...
              <thead>
                <tr className="border-b border-border bg-card">
                  {Object.keys(chart.transformedData[0]!).map(key => (
                    <th
                      key={key}
                      className="px-4 py-2 text-left font-mono font-medium text-muted-foreground"
                    >
                      {key}
                    </th>
                  ))}
